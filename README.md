...
## Lerna와 비교
yarn의 워크스페이스는 low-level-primitive라서 Lerna와 같은 도구들이 사용할 수 있다. 워크스페이스는 Lerna가 하는 만큼의 high-level 기능을 지원하지 못한다. 하지만 yarn에서 자체적으로 해결 코드 로직을 구현하고 단계를 연결하는 것으로 새로운 사용과 향상된 기능을 기대할 수 있다.
