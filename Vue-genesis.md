...
![image](https://user-images.githubusercontent.com/74655724/118623565-570fd480-b803-11eb-8ecf-02f1ee4d3d11.png)
</br>
적용 규모가 점점 커질수록 모든 사업 모듈의 모든 기능을 하나의 프로젝트로 포장하면 포장 속도가 점점 느려지고 여러 팀이 이를 유지하면 충돌이 쉽게 발생한다. 그래서 그것을 다른 서비스로 나누고 비즈니스 모듈을 세분화해야 한다. 다음은 Followme5.0 레이아웃을 사용하여 마이크로 프런트엔드 마이크로 서비스 아키텍처를 실현한다.

* 프런트엔드 레이아웃부터 좌측 네비게이션 영역과 우측 콘텐츠 영역으로 나뉘는데, 전형적인 좌우 레이아웃이다.
* 서비스 아키텍처의 관점에서는 집계 서비스와 서브 서비스로 나뉜다. 모든 요청이 집계 서비스로 이동하고 다른 URL 주소에 따라 해당 하위 서비스를 요청한다.
...
* 2019년 말 웹사이드 제품 전체를 크게 재구축하는 Followme5.0 버전을 출시한다는 사실을 알게 되면서 이 프로젝트도 등장하기 시작했다. 최근에 만들어진 것으로써 활용적인 측면에서 예시나 예제코드가 부족하여 정보를 얻기 어렵다.
* Genesis 경량 Vue SSR 라이브러리로서, 프레임워크가 아니라, 마이크로 프런트엔드, 마이크로 서비스, 정적 HTML 웹사이트, PWA 등을 생성하고자 하는 등 최저 수준의 SSR 관련 기능만 제공한다.

## Vue-genesis 구조
</br>

![image](https://user-images.githubusercontent.com/74655724/118617287-71df4a80-b7fd-11eb-844c-f77f1f17423d.png)
</br>
...
Settings과 로그인이 있다. Settings을 누르게 되면 Name과 E-mail을 입력하게 되어 새로운 계정을 생성하게 된다. 생성된 계정을 통해 로그인을 누르면 로그인이 되는 기능을 구현한다.
...
##### 실제구현화면
실제 구현화면은 Layout과 About 두 가지 영역으로 나누어져 있다. 실제구현 화면에서  Layout영역을 보게 되면 Back과 Forward버튼을 누르게 되면 Page header옆에 있는 숫자가 증가하게 된다. 실제 구현 화면에서 About영역을 보게 되면 Router link밑에 Home,About,404이 있고, 각 부분에 각각 홈화면과 login기능과 setting기능을 구현한다.
* setting화면
</br>
...
실제구현된 화면을 보게되면 예제구현에서의 모습과 다르게 나온 결과를 확인할 수 있다. 
...
* 최종적으로 Vue-gensis는 최근에 만들어진 것으로써 정보가 부족하고 따라할 수 있는 예제또한 매우 부족한 상태이다. 따라서 Vue-genesis를 포기하고 정보와 예제코드가 많은 Single-spa를 선택하게 된다.
